...
namespace AssetLib {
	...
	bool isInVector(const std::vector < AssetLib::Color4f>& vec, const AssetLib::Color4f& value, uint32_t index) {
	...
	void saveMeshV2_1(const char* fileName, const AssetLib::Model& mdl, const  AssetLib::SaveFlags& rd_only_flags) {
		...
		meshInfo.numAnimations = mdl.numAnimations;
		meshInfo.numBones = mdl.numBones;
