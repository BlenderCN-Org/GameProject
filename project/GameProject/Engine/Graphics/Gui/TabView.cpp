		...
		namespace Gui {
			...
			void TabView::update(float dt, GuiHitInfo& hitInfo, GuiItem* currentFocus) {
				if (visible) {
					...
					nextButton->updateAbsoultePos(absolutePosition.x, absolutePosition.y, absoluteSize.x, absoluteSize.y);
					nextButton->update(dt, hitInfo, currentFocus);

					prevButton->updateAbsoultePos(absolutePosition.x, absolutePosition.y, absoluteSize.x, absoluteSize.y);
					...
					if (tabs.size() > 0) {
						...
						selectedTabName->updateAbsoultePos(absolutePosition.x, absolutePosition.y, absoluteSize.x, absoluteSize.y);
						selectedTabName->update(dt, hitInfo, currentFocus);

						scrollArea->setChild(tabs[selectedTab]);
						scrollArea->updateAbsoultePos(absolutePosition.x, absolutePosition.y, absoluteSize.x, absoluteSize.y);
