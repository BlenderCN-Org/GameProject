...
extern "C"
{
	void __stdcall openglCallbackFunction(GLenum ,
		GLenum type,
		GLuint id,
		GLenum severity,
		GLsizei ,
...
void RenderEngine::setScissorRegion(int x, int y, int width, int height) {
	if (reci.renderEngineType == RenderEngineType::eRenderOpenGL) {
		...
		float px = ((float(x) / float(wx)) * 2.0F) - 1.0F;
		float py = ((float(y) / float(wy)) * 2.0F) - 1.0F;

		float sx = ((float(width) / float(wx))  * 2.0F);
		float sy = ((float(height) / float(wy)) * 2.0F);
...
void RenderEngine::printInfo(GLinfo info_in) {
	printf("%s\nGPU: %s\nDriver:%s\nGLSL: %s\n", info_in.vendor, info_in.renderer, info_in.version, info_in.shadingLanguageVersion);
}
