...
namespace Engine {
	namespace Graphics {
		namespace Gui {
			...
			ScrollArea::ScrollArea(GuiInfo& info)
				: GuiItem(info)
				, child(nullptr) 
				, offsetX(0)
				, offsetY(0)
			...
			void ScrollArea::update(float dt, GuiHitInfo& hitInfo, GuiItem* currentFocus) {
				if (visible) {
					...
					vertBar->updateAbsoultePos(absolutePosition.x, absolutePosition.y, size.x - 1, size.y);
					horzBar->updateAbsoultePos(absolutePosition.x, absolutePosition.y, size.x, size.y - 1);
					...
					if (nullptr != child) {
						...
						child->updateAbsoultePos(absolutePosition.x, absolutePosition.y, size.x, size.y);
