...
namespace Engine {
	namespace Graphics {
		namespace Gui {
			...
			void ListItem::update(float, GuiHitInfo& hitInfo, GuiItem*) {
			...
			void List::removeListItem(ListItem*) {
			...
			void List::update(float dt, GuiHitInfo& hitInfo, GuiItem* currentFocus) {
				if (visible) {
					...
					verticalScroll->updateAbsoultePos(absolutePosition.x, absolutePosition.y, size.x - 1, size.y);
					...
					for (it; it != eit; it++) {
						...
						(*it)->updateAbsoultePos(absolutePosition.x, absolutePosition.y - verticalScroll->getSelectedElement(), size.x - 15, (LIST_ITEM_SIZE));
