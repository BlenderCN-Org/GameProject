		...
		namespace Gui {
			...
			void Panel::update(float dt, GuiHitInfo& hitInfo, GuiItem* currentFocus) {
				if (visible) {
					...
					ScreenRegion sc = screenRegion;

					for (it; it != eit; it++) {
						sc.pos = absolutePosition;
						sc.size = size;
						(*it)->updateRenderRegion(sc);
			...
			void Panel::render(glm::mat4 &vpMatRef, GuiShaderContainer& shaderContainer) {
				if (visible) {
					...
					for (it; it != eit; it++) {
						...
						//const ScissorInfo scinfo = gRenderEngine->pushScissorRegion((int)screenRegion.pos.x, (int)screenRegion.pos.y, (int)screenRegion.size.x, (int)screenRegion.size.y);
