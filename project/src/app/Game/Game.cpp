...
void Game::renderScene() {
	...
	gBufferShader->useShader();
	gBufferShader->bindData(vpLocationGBuff, UniformDataType::UNI_MATRIX4X4, &vpMat);
	gBufferShader->bindData(refMatLocationGBuff, UniformDataType::UNI_MATRIX4X4, &tmpMat);
	gBufferShader->bindData(matLocationGBuff, UniformDataType::UNI_MATRIX4X4, &objMat);
	gBufferShader->bindData(clipPlane, UniformDataType::UNI_FLOAT4, &tmpVec4);
	...
	// render mirror
	glm::mat4 mirrorMdlMat = mirror->modelMatrix();
	gBufferShader->bindData(matLocationGBuff, UniformDataType::UNI_MATRIX4X4, &mirrorMdlMat);
	mirror->render();
	//
	...
	//normal.w = -20;
	glm::mat4 transposedMat = glm::transpose(glm::translate(glm::mat4(), glm::vec3(-20, 0, 0)));
	gBufferShader->bindData(matLocationGBuff, UniformDataType::UNI_MATRIX4X4, &transposedMat);
	//gBufferShader->bindData(refMatLocationGBuff, UniformDataType::UNI_MATRIX4X4, &mirrorMat);
	//gBufferShader->bindData(clipPlane, UniformDataType::UNI_FLOAT4, &normal);
	mesh->render();
...
void Game::present() {
	...
	gBufferBlit->bindData(blitEyePos, UniformDataType::UNI_FLOAT3, &cameraPos);
